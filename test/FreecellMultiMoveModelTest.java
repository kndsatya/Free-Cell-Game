...
/**
 * Perform all tests that are applicable for a Multimove model
 * using single card movements.
 */
public class FreecellMultiMoveModelTest {
  ...
  private FreecellOperations freecellMultiMoveModel;
  ...
  public void checkForObjectCorrectness() {
    ...
    try {
      gameObject = FreecellMultiMoveModel.getBuilder().cascades(0).build();
    } catch (IllegalArgumentException e) {
      //Do Nothing
    }
    ...
    try {
      gameObject = FreecellMultiMoveModel.getBuilder().cascades(3).build();
    } catch (IllegalArgumentException e) {
      //Do Nothing
    }

    try {
      gameObject = FreecellMultiMoveModel.getBuilder().cascades(9).build();
    } catch (IllegalArgumentException e) {
      //Do Nothing
    }
    ...
    try {
      gameObject = FreecellMultiMoveModel.getBuilder().opens(-1).build();
    } catch (IllegalArgumentException e) {
      //Do Nothing
    }
    ...
    try {
      gameObject = FreecellMultiMoveModel.getBuilder().opens(0).build();
    } catch (IllegalArgumentException e) {
      //Do Nothing
    }
    ...
    try {
      gameObject = FreecellMultiMoveModel.getBuilder().opens(5).build();
    } catch (IllegalArgumentException e) {
      //Do Nothing
    }
  }
  ...
  public void testStartGame() {
    FreecellOperations game = FreecellMultiMoveModel.getBuilder().build();
    ...
    int randomCascadePiles = mincascadePilesCount + r.nextInt(20);
    int randomOpenPiles = minOpenPilesCount + r.nextInt(20);
    game = FreecellMultiMoveModel.getBuilder().cascades(randomCascadePiles)
  ...
  public void testGetGameState() {
    ...
    FreecellOperations game = FreecellMultiMoveModel.getBuilder().cascades(randomCascadePiles)
            .opens(randomOpenPiles).build();
  ...
  public void testMove() {
    FreecellOperations game = FreecellMultiMoveModel.getBuilder().opens(4).build();
    List deck = game.getDeck();
    rearrangeDeck(deck);
    assertEquals(false, game.isGameOver());
    try {
    ...
    try {
      game.move(PileType.CASCADE, 0, -1, PileType.FOUNDATION,
              3);
      fail("Above line should have thrown exception");
    } catch (IllegalArgumentException e) {
      //DO Nothing
